...
namespace XBee
{
    public class XBeePacketUnmarshaler
    {
        ...
        {
            XBeeFrame frame;
            var length = (uint) (packetData[1] << 8 | packetData[2]);

            if ((length == 0) || (length > 0xFFFF))
                throw new XBeeFrameException(String.Format("Invalid Frame Lenght {0}.", length));

            if (length != packetData.Length - 4)
                throw new XBeeFrameException(String.Format("Invalid Frame Lenght - Expecting {0}, received {1}.", length, packetData.Length - 4));

            var checkData = packetData.Skip(3).ToArray();
